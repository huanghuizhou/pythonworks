...
def work(browser):
    ...
    try:
        # 输入账号和密码
        browser.find_element_by_name("username").send_keys('569181')
        ...
        if currUrl == "http://saas.800jit.com/modelhome2/applogin":
            print("success")
        else:
            print("failure1")
            # writeLog()
    ...
    time.sleep(random.random() * 1)
    browser.find_element_by_link_text("业务管理").click()

    time.sleep(random.random() * 1)
    browser.find_element_by_link_text("业务台帐").click()
    time.sleep(random.random() * 1)
    browser.find_element_by_link_text("海运出口").click()
    time.sleep(random.random() * 1)

    browser.switch_to.frame('content')  # 加载frame

    sel = browser.find_element_by_xpath("//select[@id='qybu_iscanceled']")  # 退关状态改选全部
    ...
    strattime = str(datetime.date(datetime.datetime.now().year, datetime.datetime.now().month,
                                  datetime.datetime.now().day) - datetime.timedelta(days=90))  # 默认抓取最近3个月

    choosetimezone(strattime, endtime)  # 时间段爬取，'2017-05-06'
    # choosetimezone('2013-03-01','2014-03-01')

    page = 0  # 0是第一页
    browser.get(
        "http://saas.800jit.com/modelhome2/applogin?handler=context&option=getPage&modelid=business&casenumber=securitya80aaaca281339bae77757b023d83ec47af95ebd66a1a2c9&page=pg_seaexport_search0&pagekey=business@ModelCtx@business@pg_seaexport_search0&portlet=qp_seaexport0&paging=" + str(
            page))
    html_temp = browser.execute_script("return document.documentElement.outerHTML")
    soup_temp = BeautifulSoup(html_temp, "lxml")

    getpage1 = str(soup_temp.find("span", {"class": "view-page"}))
    p = re.compile(r'页/\d+页')
    getpage2 = p.findall(getpage1)
    getpage = getpage2[0]
    intgetpage = int(getpage[2:len(getpage) - 1])
    # getpage = getpage1[getpage1.find('页/'):getpage1.find('下一页')].replace("\n", "")

    # intgetpage = int(getpage[2:len(getpage) - 2])

    for page in range(intgetpage):
        browser.get(
            "http://saas.800jit.com/modelhome2/applogin?handler=context&option=getPage&modelid=business&casenumber=securitya80aaaca281339bae77757b023d83ec47af95ebd66a1a2c9&page=pg_seaexport_search0&pagekey=business@ModelCtx@business@pg_seaexport_search0&portlet=qp_seaexport0&paging=" + str(
                page))
        ...
        for link in soup.findAll("td", {"class": "vrws-row-value", "elementname": "qp_seaexport0:businessno"}):
            ...
            businessno_urllink = businessno_urllink[
                                 businessno_urllink.find('security'):businessno_urllink.find('&page')]

            if businessno_urllink == 'security918592e2aefec562751b9ff5':
                continue
            # for page in range(2)
            geturl1 = "http://saas.800jit.com/modelhome2/applogin?handler=context&option=getPage&modelid=business&casenumber=" + businessno_urllink + "&page=pg_seaexport0"
            time.sleep(random.random() * 1)
            browser.get(geturl1)
            html2 = browser.execute_script("return document.documentElement.outerHTML")
            soupdetail = BeautifulSoup(html2, "lxml")
            # 列表页页面解析
            companyname = "宁波合羽国际物流有限公司"  # 服务商名称
            ...
            cursor.execute(sql_save1, (
                ...
                sendgoodsservice,
                landservice, customerservice, inspectservice, pawnservice, mshipper, mconsignee, mnotify, uptime,
                marks))

            db.commit()

            # 转跳分箱
            geturl2 = "http://saas.800jit.com/modelhome2/applogin?handler=context&option=getPage&modelid=business&casenumber=" + businessno_urllink + "&page=pg_setruck0&pagekey=business"
            time.sleep(random.random() * 1)
            browser.get(geturl2)
            ...
            for i in range(len(boxtype)):
                if containersno[i].find("a").string:  # 装箱号为空，默认为没有确认，不抓取
                    ...
                    containerssize_ctn = containerssize[i].find("select").find("option",
                                                                               {"selected": True}).string.replace("\n",
                                                                                                                  "").strip()
            ...
            # 转费用列表
            geturl3 = "http://saas.800jit.com/modelhome2/applogin?handler=context&option=getPage&modelid=business&casenumber=" + businessno_urllink + "&page=pg_fee_apply&pagekey=business"
            time.sleep(random.random() * 1)
            browser.get(geturl3)
...
def choosetimezone(strattime, endtime):
    js = "$('input[id=qybu_datestart0]').attr('readonly','');$('input[id=qybu_dateend]').attr('readonly','')"  # 时间控件
    browser.execute_script(js)
    browser.find_element_by_name("qybu_datestart0").clear()
    browser.find_element_by_name("qybu_datestart0").send_keys(strattime)
    browser.find_element_by_name("qybu_dateend").clear()
    browser.find_element_by_name("qybu_dateend").send_keys(endtime)
